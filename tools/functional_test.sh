...
until kubectl wait --for=jsonpath='{.status.error}'="Failure when trying to delete platform." cluster quick-test-fail-delete; do echo "wait for delete failure"; kubectl get cluster -o yaml; kubectl logs -n azimuth deployment/azimuth-caas-operator operator; sleep 2; done
