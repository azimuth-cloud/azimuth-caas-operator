...
kubectl create namespace azimuth-caas
kubectl create secret generic azimuth-sshkey --from-file=id_rsa --from-file=id_rsa.pub -n azimuth-caas
...
until kubectl wait --for=jsonpath='{.status.phase}'=Error cluster quick-test-fail-delete; do echo "wait for delete error"; kubectl get jobs; sleep 2; done
